...
class RealEmbedder(EmbedderInterface):
    """
    MiniLM-L6-v2: 80MB, 384-dim, ~0.3s per sentence on CPU.
    Semantically meaningful cosine similarity.
    Drop-in replacement for StubEmbedder — identical interface.
...
def get_embedder(use_real: bool = False, model_name: str = "sentence-transformers/all-MiniLM-L6-v2") -> EmbedderInterface:
    """Factory. Returns RealEmbedder if requested and available, else StubEmbedder."""
    if use_real:
        try:
            return RealEmbedder(model_name)
        except ImportError:
