...
class NBSISession:
    ...
    def ingest_graph(self, nodes: list[ConceptNode],
        ...
        """
        # Add nodes with embeddings — track ID remapping for merged labels
        id_remap: dict[str, str] = {}   # original_id -> canonical_id
        for node in nodes:
            if node.embedding is None and node.label:
                emb = self.embedder.encode([node.label])[0].tolist()
                node.embedding = emb
            canonical_id = self.graph.add_node(node)
            if canonical_id != node.node_id:
                id_remap[node.node_id] = canonical_id

        # Add edges — remap any IDs that were merged during add_node
        for edge in edges:
            src = id_remap.get(edge.source_id, edge.source_id)
            tgt = id_remap.get(edge.target_id, edge.target_id)
            if src == tgt:
                continue   # Merged to same node — skip self-loop
            from nbsi.graph.edge import ConceptEdge as _CE
            remapped = _CE(
                source_id=src,
                target_id=tgt,
